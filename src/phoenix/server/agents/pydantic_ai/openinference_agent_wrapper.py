        ...
        *,
        ...
        with self._tracer.start_as_current_span(
            ...
            def set_result(result: AgentRunResult[Any] | None) -> None:
                if result is None:
                    return
                output_value, mime_type = _serialize_output(result)
                span.set_attributes(get_output_attributes(output_value, mime_type=mime_type))

            yield set_result
            span.set_status(Status(StatusCode.OK))


def _serialize_output(result: AgentRunResult[Any]) -> tuple[Any, OpenInferenceMimeTypeValues]:
    output = result.output
    if isinstance(output, str):
        return output, OpenInferenceMimeTypeValues.TEXT
    return output, OpenInferenceMimeTypeValues.JSON
